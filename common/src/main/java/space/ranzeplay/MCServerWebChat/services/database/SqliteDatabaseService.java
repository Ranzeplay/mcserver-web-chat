...
/**
 * SQLite implementation of database operations for users and chat messages
...
public class SqliteDatabaseService implements IDatabaseService {
    ...
    public SqliteDatabaseService(DatabaseConfig config) {
        Path configDir = Main.getRootConfigDir().resolve("mcserver-web-chat");
        Path dbPath = configDir.resolve(config.getSqliteFilePath());
        this.databaseUrl = "jdbc:sqlite:" + dbPath.toAbsolutePath();
        
        log.info("SQLite database will be located at: {}", dbPath.toAbsolutePath());
        initializeDatabase();
    }
    ...
    public void shutdown() {
        log.debug("SQLite database service shutdown - no persistent connections to close");
    }
}